...
# Обрабатывает параграфы и удаляет абзацы с метками INSERT_XXX, если значение пустое
        ...
        for key in card_data:
            ...
            if placeholder in paragraph.text:
                if card_data[key]:  # Если значение не пустое
                    # ОСОБАЯ ОБРАБОТКА ДЛЯ КЛЮЧА "ПАН"
                    if key == "ПАН":
                        # Используем значение ПАН как ключ для поиска в paragraphs
                        pan_value = str(card_data[key]).strip()
                        logger.info(f"Обработка ПАН: значение '{pan_value}' будет использовано как ключ для поиска в paragraphs")
                        
                        if pan_value and pan_value in paragraphs:
                            # Вставляем элементы с сохранением форматирования
                            logger.info(f"Найден абзац для панели '{pan_value}'. Вставляем...")
                            insert_word_elements(doc, paragraphs[pan_value], paragraph)
                        else:
                            # Просто удаляем метку, если нет соответствующего фрагмента
                            logger.warning(f"Для значения ПАН '{pan_value}' не найден соответствующий абзац в paragraphs. Метка будет удалена.")
                            paragraph.text = paragraph.text.replace(placeholder, "")
                    # СТАНДАРТНАЯ ЛОГИКА ДЛЯ ВСЕХ ОСТАЛЬНЫХ КЛЮЧЕЙ
                    else:
            ...
            for cell in row.cells:
                ...
                for paragraph in cell.paragraphs:
                    for key in card_data:
                        ...
                        if placeholder in paragraph.text:
                            if card_data[key]:  # Если значение не пустое
                                # ОСОБАЯ ОБРАБОТКА ДЛЯ КЛЮЧА "ПАН" В ТАБЛИЦАХ
                                if key == "ПАН":
                                    # Используем значение ПАН как ключ для поиска в paragraphs
                                    pan_value = str(card_data[key]).strip()
                                    logger.info(f"Обработка ПАН в таблице: значение '{pan_value}' будет использовано как ключ для поиска в paragraphs")
                                    
                                    if pan_value and pan_value in paragraphs:
                                        # Очищаем ячейку перед вставкой
                                        cell.text = ""
                                        logger.info(f"Найден абзац для значения ПАН '{pan_value}' в таблице. Вставляем...")
                                        insert_word_elements(doc, paragraphs[pan_value], paragraph)
                                    else:
                                        paragraph.text = paragraph.text.replace(placeholder, "")
                                        logger.warning(f"Для значения ПАН '{pan_value}' в таблице не найден соответствующий абзац. Метка будет удалена.")
                                # СТАНДАРТНАЯ ЛОГИКА ДЛЯ ВСЕХ ОСТАЛЬНЫХ КЛЮЧЕЙ В ТАБЛИЦАХ
                                else:
                                    if key in paragraphs:
                                        # Очищаем ячейку перед вставкой
                                        cell.text = ""
...
@rebuild_win32com_cache_on_error
def get_document_page_count(doc_path):
    """
    Определяет количество страниц в документе Word с использованием win32com.
    Если win32com недоступен, использует эвристический метод на основе количества параграфов.
    
    :param doc_path: Путь к документу Word
    :return: Количество страниц
    """
    logger.info("=== Начало определения количества страниц ===")
    logger.info(f"Попытка определения количества страниц для документа: {doc_path}")
    
    try:
        # Проверяем, существует ли файл
        if not os.path.exists(doc_path):
            logger.error(f"Файл документа не найден по пути: {doc_path}")
            raise FileNotFoundError(f"Файл документа не найден по пути: {doc_path}")
        
        # Пытаемся использовать win32com для точного определения количества страниц
        try:
            logger.info("Попытка использовать win32com для определения количества страниц")
            import win32com.client as win32
            
            # Создаем экземпляр Word
            logger.debug("Создание экземпляра Word через win32com")
            word = win32.gencache.EnsureDispatch('Word.Application')
            word.Visible = False
            word.DisplayAlerts = False
            
            # Открываем документ
            logger.debug(f"Открытие документа: {doc_path}")
            doc = word.Documents.Open(str(doc_path))
            
            # Обновляем поля, чтобы информация о страницах была актуальной
            logger.debug("Обновление полей документа")
            doc.Fields.Update()
            doc.Repaginate()
            
            # Получаем количество страниц
            logger.debug("Получение количества страниц через ComputeStatistics")
            page_count = doc.ComputeStatistics(2)  # 2 соответствует wdStatisticPages
            
            # Закрываем документ без сохранения
            logger.debug("Закрытие документа без сохранения изменений")
            doc.Close(SaveChanges=False)
            word.Quit()
            
            logger.info(f"Успешно определено количество страниц через win32com: {page_count}")
            logger.info("=== Конец определения количества страниц (win32com) ===")
            return page_count
            
        except ImportError:
            logger.warning("Модуль win32com не установлен. Переключение на эвристический метод.")
            return estimate_page_count(doc_path)
        except Exception as e:
            logger.warning(f"Ошибка при использовании win32com: {str(e)}. Переключение на эвристический метод.")
            return estimate_page_count(doc_path)
            
    except Exception as e:
        logger.error(f"Критическая ошибка при определении количества страниц: {str(e)}")
        logger.info("Используем минимальное значение 1 для продолжения работы")
        logger.info("=== Конец определения количества страниц (с ошибкой) ===")
        return 1

def estimate_page_count(doc_path, avg_lines_per_page=35):
    """
    Эвристически оценивает количество страниц в документе на основе количества параграфов.
    
    :param doc_path: Путь к документу Word
    :param avg_lines_per_page: Среднее количество строк на странице
    :return: Оценочное количество страниц
    """
    logger.info("=== Начало эвристической оценки количества страниц ===")
    logger.info(f"Оценка количества страниц для документа: {doc_path}")
    
    try:
        logger.debug("Открытие документа через python-docx для подсчета параграфов")
        doc = Document(doc_path)
        
        # Подсчитываем количество параграфов
        total_paragraphs = len(doc.paragraphs)
        logger.debug(f"Общее количество параграфов в документе: {total_paragraphs}")
        
        # Подсчитываем количество таблиц
        total_tables = len(doc.tables)
        logger.debug(f"Общее количество таблиц в документе: {total_tables}")
        
        # Учитываем, что таблицы занимают больше места
        # Примерная оценка: каждая таблица эквивалентна 5-10 параграфам
        table_equivalent_paragraphs = total_tables * 7
        total_elements = total_paragraphs + table_equivalent_paragraphs
        
        # Оцениваем количество страниц
        estimated_pages = max(1, int(total_elements / avg_lines_per_page) + 1)
        
        logger.info(f"Эвристическая оценка: {total_paragraphs} параграфов + {total_tables} таблиц ≈ {total_elements} элементов")
        logger.info(f"При среднем {avg_lines_per_page} элементов на страницу, оценочное количество страниц: {estimated_pages}")
        
        # КОРРЕКЦИЯ: добавление 2 страниц для компенсации систематической ошибки
        # Эмпирически установлено, что эвристический метод занижает на 2 страницы
        corrected_pages = estimated_pages + 2
        logger.info(f"После коррекции (добавлено 2 страницы): {corrected_pages}")
        logger.info(f"Коррекция применена, так как эвристический метод систематически занижает количество страниц на 2 из-за таблиц и изображений")
        
        logger.info("=== Конец эвристической оценки количества страниц ===")
        return corrected_pages
    
    except Exception as e:
        logger.error(f"Ошибка при эвристической оценке количества страниц: {str(e)}")
        logger.info("Используем минимальное значение 3 для продолжения работы (после коррекции будет 5)")
        logger.info("=== Конец эвристической оценки количества страниц (с ошибкой) ===")
        return 5  # Возвращаем 5, чтобы после добавления 2 получилось 3, а после вычитания 2 в колонтитуле получится 1

def update_footer_with_page_count(doc_path):
    """
    Обновляет метку [КОЛ1] во всех колонтитулах документа на количество страниц документа минус 2.
    Сохраняет исходное форматирование шаблона: на первой странице - черный, на остальных - белый.
    
    :param doc_path: Путь к документу Word
    """
    logger.info("=== Начало обновления колонтитулов с количеством страниц ===")
    logger.info(f"Обработка документа: {doc_path}")
    
    try:
        # Получаем количество страниц
        logger.info("Запуск определения количества страниц...")
        total_pages = get_document_page_count(doc_path)
        
        # Обрабатываем случай, когда пользователь выбрал эвристический метод
        if total_pages == "USE_HEURISTIC":
            logger.info("Использование эвристического метода по выбору пользователя")
            total_pages = estimate_page_count(doc_path)
        
        logger.info(f"Общее количество страниц документа: {total_pages}")
        
        # Вычисляем количество страниц для вставки (минус 2)
        pages_to_insert = max(1, total_pages - 2)
        logger.info(f"Вычисление количества страниц для вставки: {total_pages} - 2 = {pages_to_insert}")
        
        # Открываем документ для редактирования колонтитулов
        logger.debug("Открытие документа для редактирования колонтитулов")
        doc = Document(doc_path)
        
        # Проверяем наличие секций
        if len(doc.sections) == 0:
            logger.error("В документе отсутствуют секции. Невозможно обработать колонтитулы.")
            return
        
        replacements_made = 0
        total_footers_checked = 0
        
        # Обрабатываем все секции документа
        for section_idx, section in enumerate(doc.sections):
            logger.info(f"Обработка секции {section_idx+1} из {len(doc.sections)}")
            
            # Список всех типов колонтитулов для обработки
            footer_types = [
                ("нечетных страниц", section.footer, False),  # Обычный колонтитул - НЕ первая страница
                ("первой страницы", section.first_page_footer, True),  # Колонтитул первой страницы
                ("четных страниц", section.even_page_footer, False)   # Колонтитул четных страниц - НЕ первая страница
            ]
            
            # Обрабатываем каждый тип колонтитула
            for footer_name, footer, is_first_page in footer_types:
                # Проверяем, существует ли колонтитул
                if footer is None:
                    logger.debug(f"Колонтитул {footer_name} не существует в секции {section_idx+1}")
                    continue
                    
                total_footers_checked += 1
                logger.debug(f"Проверка {footer_name} колонтитула секции {section_idx+1}")
                
                # Проверяем, есть ли текст в колонтитуле
                footer_has_content = any(paragraph.text.strip() for paragraph in footer.paragraphs)
                logger.debug(f"{footer_name.capitalize()} колонтитул содержит текст: {footer_has_content}")
                
                # Ищем и заменяем метку [КОЛ1] во всех абзацах колонтитула
                for i, paragraph in enumerate(footer.paragraphs):
                    original_text = paragraph.text
                    logger.debug(f"Проверка параграфа {i+1} {footer_name} колонтитула: '{original_text}'")
                    
                    if "[КОЛ1]" in original_text:
                        logger.info(f"Найдена метка [КОЛ1] в {footer_name} колонтитуле, параграф {i+1}")
                        logger.debug(f"Старый текст: '{original_text}'")
                        
                        # === ПЕРЕРАБОТАННЫЙ ПОДХОД: Сохраняем существующее форматирование ===
                        logger.info("Замена метки с сохранением форматирования...")
                        ...
                        full_text = ''.join([run['text'] for run in runs_formatting])
                        
                        # Заменяем метку в полном тексте
                        new_full_text = full_text.replace("[КОЛ1]", str(pages_to_insert))
                        
                        # Очищаем все runs
                        for run in paragraph.runs:
                            run.text = ''
                        
                        # Восстанавливаем текст с сохранением форматирования
                        # Для простоты создаем один run с новым текстом, но сохраняем форматирование первого run
                        if runs_formatting:
                            # Берем форматирование из первого run, который содержал метку
                            first_run_formatting = runs_formatting[0]
                            ...
                            # КРИТИЧЕСКОЕ ИЗМЕНЕНИЕ: НЕ меняем цвет - сохраняем исходный из шаблона
                            if first_run_formatting['font_color']:
                                new_run.font.color.rgb = first_run_formatting['font_color']
                                logger.debug(f"Сохранен исходный цвет текста: {first_run_formatting['font_color']}")
                            else:
                                logger.debug("Исходный цвет текста не был установлен (None)")
                            
                            logger.debug(f"Форматирование восстановлено: размер {first_run_formatting['font_size']}, цвет {first_run_formatting['font_color']}")
                        
                        logger.info(f"Метка заменена на '{pages_to_insert}' с сохранением форматирования")
                        logger.debug(f"Тип колонтитула: {'первая страница' if is_first_page else 'не первая страница'}")
                        
                        replacements_made += 1
        
        # Сохраняем изменения
        logger.debug("Сохранение изменений в документе")
        doc.save(doc_path)
        
        if replacements_made > 0:
            logger.info(f"Успешно обновлено колонтитулов: {replacements_made} из {total_footers_checked}")
            logger.info(f"Все найденные метки [КОЛ1] заменены на {pages_to_insert}")
            logger.info("Форматирование сохранено из шаблона: цвет остается как в исходном документе")
        else:
            logger.warning("Метка [КОЛ1] не найдена ни в одном колонтитуле документа")
            logger.info("Возможные причины:")
            logger.info("1. Метка написана с опечаткой или в другом регистре")
            logger.info("2. Документ еще не содержит финального количества страниц")
            logger.info("3. Метка была удалена предыдущими обработками")
        
        logger.info("=== Конец обновления колонтитулов с количеством страниц ===")
        
    except Exception as e:
        logger.exception(f"Критическая ошибка при обновлении колонтитулов: {str(e)}")
        logger.info("=== Конец обновления колонтитулов с количеством страниц (с ошибкой) ===")
        raise

def verify_footer_update(doc_path):
    """
    Проверяет, была ли успешно заменена метка [КОЛ1] во всех колонтитулах.
    
    :param doc_path: Путь к документу Word
    :return: True, если проверка пройдена успешно
    """
    logger.info("=== Начало проверки обновления колонтитулов ===")
    logger.info(f"Проверка документа: {doc_path}")
    
    try:
        # Получаем количество страниц для проверки
        total_pages = get_document_page_count(doc_path)
        pages_to_insert = max(1, total_pages - 2)
        ...
        if len(doc.sections) == 0:
            logger.error("В документе отсутствуют секции. Невозможно проверить колонтитулы.")
            return False
        ...
        for section_idx, section in enumerate(doc.sections):
            ...
            footer_types = [
                ("обычный", section.footer),
                ("первой страницы", section.first_page_footer),
                ("четных страниц", section.even_page_footer)
            ]
            ...
            for footer_name, footer in footer_types:
                ...
                for i, paragraph in enumerate(footer.paragraphs):
                    text = paragraph.text
                    logger.debug(f"Параграф {i+1} {footer_name} колонтитула: '{text}'")
                    
                    if "[КОЛ1]" in text:
                        logger.warning(f"Метка [КОЛ1] все еще присутствует в {footer_name} колонтитуле: '{text}'")
                        markers_found += 1
                    elif str(pages_to_insert) in text:
                        logger.info(f"Найдено значение {pages_to_insert} в {footer_name} колонтитуле: '{text}'")
                        found_replacements += 1
        
        if markers_found == 0 and found_replacements > 0:
            logger.info(f"Проверка успешна: найдено замен в {found_replacements} колонтитулах")
            return True
        elif markers_found > 0:
            logger.warning(f"Обнаружено {markers_found} незамененных меток [КОЛ1] в колонтитулах")
            return False
        else:
            logger.warning("Не найдено замен и меток в колонтитулах (возможно, метки отсутствуют в шаблоне)")
            return True  # Считаем проверку успешной, так как меток просто нет в шаблоне
            
    except Exception as e:
        logger.exception(f"Ошибка при проверке колонтитулов: {str(e)}")
        return False
    finally:
        logger.info("=== Конец проверки обновления колонтитулов ===")
...
def insert_images(doc_path, images_folder, card_data, brightness_factor=1.5, contrast_factor=1.5):
    ...
    elif card_value == "СВО_Молов_образец_прямая идентификация":
    ...
    card_value = card_data.get("1", "").strip()
    use_custom_width_for_1_2_3 = card_value in [
...
def read_and_filter_txt_data(file_path):
    """ Читает данные из txt файла и возвращает все профили, оканчивающиеся на -26 """
    logger.info(f"Начало чтения и фильтрации данных из файла: {file_path}")
    ...
    # Фильтруем данные, оставляя только профили, оканчивающиеся на -26
    filtered_data = [line for line in data if line[0].strip().endswith("-26")]
    ...
    for profile_name, profile_data in profiles.items():
        ...
        for line in profile_data:
            ...
            for i in range(5, 9):  # Столбцы 5-8: Allele 1-4
    ...
    # Если найдены многоллельные профили, запрашиваем подтверждение
...
def insert_table_data(doc_path, profiles_data, card_data, custom_headers=None):
    """
    ...
    # Блок проверки наличия y и rs при Amel X
    logger.info("Проверка наличия значений в Y/RS локусах для профилей с Amel X")
    ...
    for profile_name, profile_data in profiles_data.items():
        ...
        for line in profile_data:
            ...
            # Проверяем, является ли локус Amel или Amelogenin
            # (для надёжности также проверяем начало с AMEL)
        ...
        if amel_line is not None:
            # Получаем аллели из столбцов 5 и 6 (индексы 5 и 6)
            alleles = []
            for idx in [5, 6]:
            ...
            if has_y:
                ...
                continue  # Переходим к следующему профилю
            else:
                logger.debug(f"Профиль {profile_name} имеет Amel без Y (только X или отсутствие), проверяем Y/RS локусы")
                # Проверяем Y/RS локусы
    ...
    logger.info("Проверка Y/RS локусов завершена успешно")
    ...
    if card_value in [
        ...
        table_width = 5.0
        logger.info("Установлена ширина таблицы: 6.0 дюймов")
    elif card_value.startswith("ЭКЦ_образец_"):
        table_width = 6.0
        logger.info("Установлена ширина таблицы: 7.0 дюймов для ЭКЦ образца")
    else:
        table_width = 7.0
        logger.info("Установлена ширина таблицы: 8.0 дюймов (по умолчанию)")
                ...
                if "[TABLE_DATA]" in cell.text:
                    ...
                    for locus in sorted_loci:
                        ...
                        for i, profile_name in enumerate(profile_names):
                            ...
                            if i + 1 < len(new_row.cells):
                                new_row.cells[i+1].text = alleles
                    ...
                    # СТАРАЯ ЛОГИКА ДЛЯ ВСЕХ ОСТАЛЬНЫХ СЛУЧАЕВ
                        ...
                        if has_dash and has_f:
                            if card_data.get("1") in ["СВО_Молов_образец_родственники", 
                                                    "СВО_Молов_образец_прямая идентификация", 
                                                    "СВО_Ростов_образец_родственники", 
                                                    "СВО_Ростов_образец_прямая идентификация", 
                                                    "ЭКЦ"]:
...
def process_ekc_sample_comparison(card_data, working_materials_folder, original_txt_path):
    ...
    Возвращает кортеж (успех_обработки, временный_путь_к_файлу, нужно_остановиться, профили_данные, кастомные_заголовки)
    """
    logger.info("Начало обработки сравнения с ЭКЦ образцом...")
    
    template_value = card_data.get("1", "").strip()
    
    # Проверяем, нужно ли обрабатывать ЭКЦ образец
    if not template_value.startswith("ЭКЦ_образец_"):
    ...
    logger.info(f"Обнаружен ЭКЦ образец для обработки: {template_value}")
    ...
    try:
        ...
        if sample_data_lines and len(sample_data_lines) > 1:  # Пропускаем заголовок
            for line in sample_data_lines[1:]:  # Начинаем со второй строки (после заголовка)
                parts = line.strip().split('\t')
                if len(parts) > 0 and parts[0].strip().endswith("-26"):
                    sample_real_profile_name = parts[0].strip()
                    logger.info(f"Извлечен номер образца из данных файла: '{sample_real_profile_name}'")
                    break
        
        if not sample_real_profile_name:
            # Если не нашли в данных, пытаемся извлечь из заголовка или использовать fallback
            logger.warning("Не удалось извлечь номер образца из данных файла. Используем альтернативные методы...")
            
            # Пытаемся найти номер в заголовке
            import re
            match = re.search(r'(\d+-26)', sample_profile_title)
            if match:
                sample_real_profile_name = match.group(1)
                logger.info(f"Извлечен номер образца из заголовка: '{sample_real_profile_name}'")
            else:
                # Fallback: берем первый профиль, оканчивающийся на -26 из файла образца
                for line in sample_data_lines[1:]:
                    parts = line.strip().split('\t')
                    if len(parts) > 0 and parts[0].strip().endswith("-26"):
                        sample_real_profile_name = parts[0].strip()
                        logger.info(f"Используем fallback номер образца: '{sample_real_profile_name}'")
                        break
        
        if not sample_real_profile_name:
            raise ValueError("Не удалось определить номер ЭКЦ образца из файла")
        
        logger.info(f"Окончательный номер ЭКЦ образца для сравнения: '{sample_real_profile_name}'")
        
        # === 3. СОЗДАНИЕ ВРЕМЕННОГО ФАЙЛА С ОБЪЕДИНЕННЫМИ ПРОФИЛЯМИ ===
        temp_txt_path = working_materials_folder / f"temp_combined_{card_data['НОМ']}.txt"
        logger.info(f"Создание объединенного профиля: {original_txt_path} + {sample_txt_path} -> {temp_txt_path}")
        ...
        with open(temp_txt_path, 'w', encoding='utf-8') as f:
            # Записываем заголовок из рабочего файла
            f.write(working_lines[0])
            
            # Записываем данные из рабочего файла (кроме заголовка)
            for line in working_lines[1:]:
                f.write(line)
            
            # Записываем данные из файла-образца (кроме заголовка и строки с заголовком профиля)
            for line in sample_data_lines[1:]:
                f.write(line)
        
        logger.info(f"Объединенный файл создан: {temp_txt_path}")
        
        # === 4. ЧТЕНИЕ И ФИЛЬТРАЦИЯ ДАННЫХ ИЗ ВРЕМЕННОГО ФАЙЛА ===
        logger.info(f"Чтение данных из временного файла: {temp_txt_path}")
        profiles_data = read_and_filter_txt_data(temp_txt_path)
        logger.info(f"Найдены профили в объединенном файле: {list(profiles_data.keys())}")
...
def check_case_number(card_data, txt_path, image_path, foregram_path, working_materials_folder): # Добавлен аргумент
    ...
    # Также преобразуем в строки, если вдруг None
    ...
    if (case_number_from_card != case_number_from_image or
        ...
        # Собираем очищенные значения для сообщения об ошибке
        ...
        logger.error(f"[ERROR] {error_msg}") # Печатаем ошибку в лог
        raise ValueError(error_msg) # Вызываем исключение с очищенной информацией
...
def check_txt_filename_consistency(txt_path, profiles_data, case_number):
    ...
    # Проверяем все профили на соответствие
    exact_matches = []
    flexible_matches = []
...
def check_consistency(card_data, profiles_data, image_path, foregram_path, txt_path, working_materials_folder):
    ...
    # Находим профили, которые соответствуют номеру заключения
    matching_profiles = []
    ...
    if not matching_profiles:
        for profile_name in profiles_data.keys():
            # Проверяем, начинается ли профиль с номера заключения и заканчивается на "-26"
...
def main():
    try:
        ...
        if card_data.get("1") in [
            ...
            # Обработка ЭКЦ образцов (сравнение профилей)
            temp_txt_path = txt_path
            should_stop = False
            profiles_data = None
            custom_headers = {}  # Добавляем переменную для кастомных заголовков
            matching_profile_name = None  # Добавляем переменную для имени совпавшего профиля

            if not card_data.get("1") in [
                "СВО_кость_нет результата",
                "СВО_Молов_образец_родственники_нет результата_форез", 
                "СВО_Молов_образец_родственники_нет результата_RT",
                "ЭКЦ_нет результата"
            ]:
                # Проверяем только для экспертиз с генотипом
                logger.info("Проверка на наличие ЭКЦ образца для сравнения...")
                ...
                # Если не ЭКЦ образец, читаем данные обычным способом
                profiles_data = read_and_filter_txt_data(temp_txt_path)
        ...
        logger.info("=== Начало дополнительной обработки колонтитула ===")
        try:
            # Сначала обновляем колонтитул
            update_footer_with_page_count(output_path)
            
            # Затем проверяем, успешно ли прошла замена
            if verify_footer_update(output_path):
                logger.info("Проверка обновления колонтитула прошла успешно")
            else:
                logger.warning("Проверка обновления колонтитула не прошла успешно. Возможно, замена не выполнена корректно.")
                # Можно добавить дополнительные действия при неудачной проверке
                # Например, повторную попытку или уведомление пользователя
                
            # Дополнительная проверка: попробуем определить количество страниц еще раз
            final_page_count = get_document_page_count(output_path)
            logger.info(f"Финальное количество страниц документа после всех изменений: {final_page_count}")
            
        except Exception as e:
            logger.error(f"Критическая ошибка при обработке колонтитула: {str(e)}")
            logger.exception("Полный трейсбэк ошибки:")
            # Можно добавить уведомление пользователю, что колонтитул не обновлен
            # Но не прерываем выполнение программы, так как это не критично для основной функциональности
        logger.info("=== Конец дополнительной обработки колонтитула ===")
