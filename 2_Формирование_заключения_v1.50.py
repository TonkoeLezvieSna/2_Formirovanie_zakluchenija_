...
def insert_table_data(doc_path, profiles_data, card_data, custom_headers=None, skip_comment=False,
    ...
    if card_value in [
        ...
        table_width = 6.0
        logger.info("Установлена ширина таблицы: 7.0 дюймов для тобольского образца")
    else:
        table_width = 7.0
...
def process_tobolsk_sample(card_data, working_materials_folder, original_txt_path):
    ...
    # Определяем реальное имя профиля образца (должно оканчиваться на -26)
    sample_real_profile_name = None
    ...
    if not sample_real_profile_name:
        raise ValueError("Не удалось определить имя профиля образца (должно оканчиваться на -26)")
    logger.info(f"Имя профиля образца: {sample_real_profile_name}")
    ...
    try:
        ...
        if card_data.get("1") in [
            ...
            if template_value in no_genotype_templates:
                ...
                # Сначала проверяем шаблон Тобольск
                if template_value == "Тобольск_образец_мужчина":
                ...
                # Иначе проверяем ЭКЦ образец
                elif template_value.startswith("ЭКЦ_образец_"):
            ...
            skip_comment_flag = (card_data.get("1", "").strip() == "Тобольск_образец_мужчина")
